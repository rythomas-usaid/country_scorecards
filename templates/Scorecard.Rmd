---
title: "`r paste0('FTF FY 2023 Performance Scorecard ',paste0(trimws(gsub('USAID ','',gsub('\\(.*$','',params$program )))) )`"
output: 
  word_document: 
...
ix <- which(str_detect(params$ou_names, "USAID"))
usaid_ou <- params$ou_names[ix]
n_activities <- params$active_activities_unique %>% filter(ou %in% params$ou_names) %>%
...
finance_sex_dna <- params$extract %>%
  filter(ou %in% params$ou_names & year == 2023 & ic=="EG.3.2-27" & udn %in% dna_udns) %>%
  summarize(dna = sum_(actual))
finance_sex_mixed <- params$extract %>%
  filter(ou %in% params$ou_names & year == 2023 & ic=="EG.3.2-27" & udn %in% other_udns) %>%
  summarize(mixed = sum_(actual))
total_financing <- params$extract %>%
  filter(ou %in% params$ou_names & year == 2023 & ic=="EG.3.2-27" & udn == "3") %>%
  summarise(total = sum_(actual)) %>% "["("total")
ou_dat <- params$pt_dat %>% filter(ou %in% ou_names) %>% arrange(ro, ou, year, type, name)

section1 <- s1_plot(dat = params$input_dat
                    , ou_names = params$ou_names
...
This Feed the Future (FTF) Performance Scorecard assesses the `r paste0(trimws(gsub("\\(.*$", "", params$ou_names )))` progress toward FY 2030 Performance Targets.
...
This section shows PT scores for each applicable PT for `r paste0(trimws(gsub("\\(.*$", "", params$ou_names )))`.
...
plot_df <- params$national_df %>%
    filter(ou == trimws(gsub("\\(.*$", "", gsub("USAID ", "",  params$program )))) %>%
...
p_ati <- ati_plot(params$ati, params$program)
p_ati
```
...
```{r}
if(params$program == "Bureau for Resilience and Food Security (RFS)") {
...
# Budget --------
# ou_budget <- budget %>% filter(year %in% 2020:2024 & ou == params$program) %>%
...
make_ftf_budget_plot(ftf_budget, ou_name = params$program)
...
This section highlights the `r min(n_activities, 5)` activities (out of the total `r n_activities` in the `r params$program` program portfolio) that have the highest level of FY 2023 annual disbursements and highest reported values for the relevant indicators.

```{r}
t5d <- params$top5_disbursements %>% 
  filter(ou_dis == usaid_ou) %>% 
...
params$top5_results %>%
  ...
  filter(ou == usaid_ou) %>% select(-ou) %>% 
  make_human_readable_names() %>% 
  knitr::kable(table.attr = "style='width:8in;'")
```

```{r}
knitr::include_graphics(paste0("../", params$mapfile))
