...
targets <- read_sheet("13UfrGUnaDJbCO-Nz6tP0T1ORrJcVAeELGZK7WLKTJ6c", sheet = "targets_long") %>%
  filter(! str_starts("_", ou) ) %>%
  inner_join(ftf_programs)
...
group_target_table <- input_dat %>%
  filter(!is.na(actual) & year %in% 2023 & ! country_program %in% sales_ous_to_program$programs) %>%
  distinct(country_program) %>%
  mutate(`PT1: Sales` = "X")  %>%
  full_join(input_dat %>%
              filter(!is.na(actual) & year %in% 2023 & ! country_program %in% gf_ous_to_program$programs) %>%
              distinct(country_program) %>%
              mutate(`PT2: Gender financing ratio` = "X")) %>%
  full_join(input_dat %>%
              filter(!is.na(actual) & year %in% 2023 & ! country_program %in% ht_ous_to_program$programs) %>%
              distinct(country_program) %>%
              mutate(`PT3: Climate hectares` = "X")) %>%
  full_join(input_dat %>%
              filter(!is.na(actual) & year %in% 2023 & ! country_program %in% psi_ous_to_program$programs) %>%
              distinct(country_program) %>%
              mutate(`PT4: Private sector investment` = "X"))
googlesheets4::write_sheet(group_target_table, "1ynvPDs5RqmfUbSFlcTlCry7z4dRnFnMBR37S_GtvNLw"
                           , sheet = "Group Target Contributors by Country Program")

## Define params -----------
### Sales ------------------
sales <- input_dat %>%
  filter(year == 2023 ) %>%
  sales_(level = "scorecard_program") %>% filter(name == "actual") %>%
  group_by(scorecard_program, year, type = name) %>%
  summarize(`PT1: Sales` = value)
...
sales_initiative <- input_dat %>%
  filter(year== 2023) %>%
  sales_(level = "initiative") %>% filter(name == "actual") %>%
  select(year, type = name, `PT1: Sales` = value) %>% group_by(year, type) %>%
  summarize(scorecard_program = "FTF Initiative", `PT1: Sales` = sum_(`PT1: Sales`))
sales <- bind_rows(sales_initiative, sales)

### GF ratio  ------------------
gf <- input_dat %>%
  filter(year == 2023) %>%
  gender_financing_(level = "scorecard_program") %>% filter(name == "actual") %>%
  select(scorecard_program, type = name, year, `PT2: Gender financing ratio` = value)

# Add initiative level
gf_initiative <- input_dat %>%
  filter(year == 2023) %>%
  gender_financing_(level = "initiative") %>% filter(name == "actual") %>%
  select(type = name, year, `PT2: Gender financing ratio` = value) %>%
  mutate(scorecard_program = "FTF Initiative", .before = everything())
gf <- bind_rows(gf_initiative, gf)

### HT ------------------
ht <- input_dat %>%
  filter( year == 2023) %>%
  hectares_(level = "scorecard_program") %>% filter(name == "actual") %>%
  select(scorecard_program, type = name, year, `PT3: Climate hectares` = value)
...
ht_initiative <- input_dat %>%
  ...
  select(scorecard_program, type = name, year, `PT3: Climate hectares` = value) %>%
  group_by(type, year) %>%
  summarise(scorecard_program = "FTF Initiative"
            , `PT3: Climate hectares` = sum_(`PT3: Climate hectares`)
            , .groups = "drop")
ht <- bind_rows(ht_initiative, ht)

### PSI ------------------
psi_initiative <- input_dat %>% psi_(level = "initiative") %>%
  filter(name %in% c("actual") & year == 2023 & !is.na(value)) %>%
  rename(type = name, `PT4: Private sector investment` = value) %>%
  select(-c(ic)) %>% mutate(scorecard_program = "FTF Initiative", .before=everything())
psi <- input_dat %>% psi_(level = "scorecard_program") %>%
  filter(str_detect(name, "actual_3y") & year == 2023
         & !is.na(value)) %>%
  rename(type = name, `PT4: Private sector investment` = value) %>%
  # pivot_wider() %>%
  mutate(type = str_remove(type, "_3y")) %>%
  select(-c(ic))
# psi_baseline <- bind_rows(psi, filter(psi_initiative, type == "actual" & year == 2022)) %>%
#   select(scorecard_program, type, year, starts_with("PT4")) %>%
#   filter(year==2022 & type == "actual") %>% mutate(type = "baseline")
psi <- bind_rows(filter(psi_initiative, !(type == "actual" & year == 2022)), psi)
...
mddw <- read_sheet("13UfrGUnaDJbCO-Nz6tP0T1ORrJcVAeELGZK7WLKTJ6c", sheet = "mddw_scorecard") %>%
  select(-date) %>% filter(type == "actual") %>%
  left_join(ftf_programs) %>% select(-c(ou, ro, country_program)) %>%
  rename(`PT5: MDD-W` = value)
...
pt_dat <- sales %>% full_join(gf) %>% full_join(ht) %>% full_join(psi) %>%
  ungroup() %>% select(-year) %>%
  full_join(mddw %>% filter(type != "outyear")) %>%
  bind_rows(
    targets %>% select(-c(ro, ou, country_program)) %>% drop_na() %>%
      # filter(! str_starts(name, "PT5")) %>%
      pivot_wider(names_from = name, values_from = value)
    )  %>%
  # bind_rows(psi_baseline) %>%
  pivot_longer(-c(scorecard_program, type)) %>% drop_na(value) %>%
  arrange(scorecard_program, name, type) %>% ungroup()
...
pt_upload <- pt_dat %>%
  ...
  mutate(name = ifelse(name %in% psi_names, "PT4: Private sector investment", name)) %>%
  dplyr::rowwise() %>%
  mutate(
    name = ifelse(name %in% psi_names, "PT4: Private sector investment" , name)
    , interim_target = ifelse(
    ...
    # For PT4, the initiative has a 7.5% increase on the annual figures, while the
    # individual programs that are 3-year averages have a 2.88% increase
    , ifelse(name %in% psi_names & scorecard_program != "FTF Initiative", baseline * 1.075
    , ifelse(name %in% psi_names & scorecard_program == "FTF Initiative", baseline * 1.0288
  ...
  mutate(across(c(baseline, actual, interim_target), ~ . / interim_target, .names = "{.col}_norm")
         , across(-c(scorecard_program, name), ~ ifelse(is.finite(.), ., NA))
         , absolute_difference = actual - interim_target

         , scorecard_program_track = ifelse(
           scorecard_program == "Nigeria" & name == "PT1: Sales", "Not Available"
           , ifelse(scorecard_program == "USDA" & str_starts(name, "PT3"), "Not available"
           # If no performance target, set to Not applicable
           , ifelse(is.na(`performance target`) & !str_starts(name, "PT4"), "Not applicable"
           # , ifelse(is.na(`performance target`)), "Not applicable"
           , ifelse(is.na(`performance target`) & str_starts(name, "PT4"), "Not available"

           # For PT1-PT4, any scorecard_program with a performance target with a 0 or NA for FY23 is not on track
...
finance_total <- extract %>% gender_financing_(level = "scorecard_program") %>%
  filter(name == "actual", year ==2023) %>%
  group_by(scorecard_program, year) %>%
